                    ...
                    <div class="col-4">
                        <div class="card bg-dark btn-container border-light d-flex">
                            <div class="card-body">
                                ...
                                <p class="p-0 m-0">عدد الملفات: {{ count(Storage::disk('app')->files("/files/$file->url")) }}</p>
                                <p class="p-0 m-0">عدد مرات التحميل: {{ $file->downloads }}</p>
                                <p>الناشر: {{ User::where('id', session('user_id'))->first()->name }}</p>
                                <button class="btn btn-outline-info"
                                    onclick="download('{{ URL::temporarySignedRoute('download.folder', now()->addHours(1), ['url' => $file->fakeurl]) }}')"><i
                                        class="fa-solid fa-download"></i> تحميل</button>
                                ...
                                <button class="btn btn-danger"
                                    onclick="download('{{ URL::temporarySignedRoute('File.delete', now()->addMinutes(15), ['url' => $file->fakeurl]) }}')"><i
